...
test('public host policy accepts the canonical hostname and configured aliases across the trusted proxy', () => {
  const allowed = createHostPolicy('https://access.zipkin.dev', 'arcade.zipkin.dev');
  for (const host of ['access.zipkin.dev', 'access.zipkin.dev:443',
    'arcade.zipkin.dev', 'arcade.zipkin.dev:443', 'ARCADE.ZIPKIN.DEV']) assert.equal(allowed(host), true, host);
});

test('public host policy rejects unconfigured names, ports, credentials, paths, and malformed aliases', () => {
  const allowed = createHostPolicy('https://access.zipkin.dev', 'arcade.zipkin.dev');
  for (const host of ['evil.example', 'arcade.zipkin.dev:44334', 'arcade.zipkin.dev:444', 'arcade.zipkin.dev/path',
...
test('write requests accept the exact browser origin that served the page', () => {
  const publicOrigin = 'https://access.zipkin.dev';
  ...
  assert.equal(matchesRequestOrigin('https://arcade.zipkin.dev', publicOrigin, allowed), true);
  for (const origin of ['https://evil.example', 'https://access.zipkin.dev:44334', 'http://access.zipkin.dev',
